...
from tracking_utils.NSA_kalman_filter import KalmanFilter
from models import *
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, vis_bbox=None, temp_feat=None, buffer_size=30):
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        ...
        # )
        self.mean, self.covariance = self.kalman_filter.update(
            self.mean, self.covariance, self.tlwh_to_xyah(new_track.tlwh), self.score  # NSA
        )
    ...
    def update(self, new_track, frame_id, update_feature=True):
        ...
        self.mean, self.covariance = self.kalman_filter.update(
            self.mean, self.covariance, self.tlwh_to_xyah(new_tlwh), self.score)  # NSA
        self.state = TrackState.Tracked
        self.is_activated = True
    ...
    def update_vis(self, new_track, frame_id, update_feature=True):
        """
        Update a matched track
        :type new_track: STrack
        :type frame_id: int
        ...
        self.score = new_track.score
        self.vis_bbox = new_track.vis_bbox
        if update_feature:
            self.update_features(new_track.curr_feat)
...
class VGTASTracker(object):
    ...
    def update(self, im_blob, img0):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], temp_feat=f, buffer_size=30) for
        ...
        # dists = matching.Biou_distance(r_tracked_stracks, detections, b=0.3)
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], temp_feat=f, buffer_size=30) for
        ...
        # dists = matching.Biou_distance(second_tracked_stracks, detections_second, b=0.5)
